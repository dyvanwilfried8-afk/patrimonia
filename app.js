...
return `<div class="cat-card" onclick="navigate('${cat.id === 'savings' ? 'savings' : 'portfolio'}')"> <div class="cat-icon" style="background:${cat.color}22;">${cat.icon}</div> <div class="cat-info"><div class="cat-name">${cat.label}</div><div class="cat-sub">${pct}% du patrimoine</div></div> <div class="cat-right"><div class="cat-val">${fmt.format(val)}</div></div> </div>`;
}).filter(Boolean);
...
return `<tr><td><div class="asset-name">${displayName}${tickerBadge} <span class="asset-badge">${typeLabels[a.type]||''}</span></div></td> <td style="color:var(--muted2);font-size:12px;">${a.source||'manuel'}</td> <td>${fmt.format(val)}</td> <td class="${pc}">${pnl>=0?'+':''}${fmt.format(pnl)}</td> <td class="${pc}">${pnl>=0?'+':''}${pnlP}%</td> <td class="perf-zero">–</td><td class="perf-zero">–</td><td class="perf-zero">–</td><td class="perf-zero">–</td> <td class="${pc}">${pnl>=0?'+':''}${pnlP}%</td> <td style="color:var(--muted2);">${poids}%</td></tr>`;
}).join(’’);
}
...
if (list) list.innerHTML = savings.length ? savings.map((sv,i)=>` <div class="fee-item"> <div><div style="font-size:14px;font-weight:500;">${sv.name}</div><div style="font-size:12px;color:var(--muted2);">Taux : ${sv.rate||0}%</div></div> <div style="text-align:right;"><div style="font-weight:600;">${fmt.format(sv.balance||0)}</div> <button onclick="deleteSavings(${i})" style="font-size:11px;color:var(--danger);background:none;border:none;cursor:pointer;">Supprimer</button></div> </div>`).join(’’) : ‘<div class="empty-state"><div class="icon">🏦</div><p>Aucun livret</p></div>’;
...
if (list) list.innerHTML = expenses.length ? expenses.map((e,i)=>` <div class="fee-item"> <div><div style="font-size:13px;font-weight:500;">${e.label}</div><div style="font-size:11px;color:var(--muted2);">${e.category||''}</div></div> <div style="display:flex;align-items:center;gap:12px;"><span style="font-weight:600;">${fmt.format(e.amount||0)}/mois</span> <button onclick="deleteExpense(${i})" style="font-size:11px;color:var(--danger);background:none;border:none;cursor:pointer;">✕</button></div> </div>`).join(’’) : ‘<div class="empty-state"><div class="icon">💳</div><p>Aucune dépense fixe</p></div>’;
}
...
return `<div style="padding:10px 16px;background:var(--surface2);border-radius:8px;text-align:center;min-width:100px;"> <div style="font-size:11px;color:var(--muted2);">Objectif</div> <div style="font-size:15px;font-weight:600;color:${c};">${fmt.format(t)}</div> <div style="font-size:11px;color:${c};">${yr!==null?'Année '+yr:'Non atteint'}</div></div>`;
}).join(’’);
}
...
return `<div class="fee-item"><div class="fee-score" style="background:${dot};"></div> <div style="flex:1;"><div style="font-size:13px;font-weight:500;">${a.name||'–'}</div><div style="font-size:11px;color:var(--muted2);">${a.fees||0}% / an</div></div> <div style="text-align:right;"><div style="font-weight:600;color:var(--danger);">${fmt.format(fee)}/an</div><div style="font-size:11px;color:var(--muted2);">${fmt.format(val)} investi</div></div></div>`;
}).join(’’);
}
...
let html = ‘’;
if (totalAnnual > 0) {
html += `<div style="margin-bottom:12px;padding:12px;background:rgba(34,197,94,0.06);border:1px solid rgba(34,197,94,0.15);border-radius:8px;"> <div style="font-size:11px;color:var(--muted2);margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;">Dividendes estimés / an</div> <div style="font-size:22px;font-weight:600;color:var(--green);">${fmt.format(totalAnnual)}</div> <div style="font-size:11px;color:var(--muted2);">soit ${fmt.format(totalAnnual/12)}/mois</div> </div>`;
...
rows += `<tr> <td style="font-weight:500;">${bank}</td> <td style="color:var(--accent2);">${taux}%</td> <td style="font-weight:600;color:var(--accent2);">${capMax>0?fmt.format(capMax):'–'}</td> <td>${montantEmprunt>0?fmt.format(mensualite)+'/mois':'–'}</td> <td style="color:${color};">${tauxEndet}%</td> <td style="color:var(--muted2);font-size:12px;">${coutTotal>0?fmt.format(coutTotal):'–'}</td> <td><span class="badge ${feasible?'badge-up':'badge-down'}">${feasible?'✓ Possible':'✗ Serré'}</span></td> </tr>`;
});
