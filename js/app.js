...
function getSbConfig() {
  try {
    return JSON.parse(localStorage.getItem('patrimonia_supabase') || 'null');
  } catch { return null; }
}
...
async function deleteAccount() {
  ...
  try {
    // 1. Supprimer les données cloud si Supabase configuré
    const cfg = getSbConfig();
    if (cfg) {
      try {
        await sbRequest('DELETE', `patrimonia_data?username=eq.${encodeURIComponent(currentUser)}`);
      } catch(e) { console.warn('Cloud delete failed:', e); }
    }

    // 2. Supprimer toutes les données localStorage de cet utilisateur
    const keysToDelete = [];
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (k && (k.startsWith(`wv_${currentUser}_`) || k === 'wv_users' || k === 'pat_users')) {
        keysToDelete.push(k);
      }
    }

    // Supprimer le compte des utilisateurs
    try {
    ...
    // Supprimer toutes les clés de données utilisateur
    keysToDelete.filter(k => k !== 'wv_users').forEach(k => localStorage.removeItem(k));

    // 3. Déconnexion
    sessionStorage.removeItem('wv_user');
    toast('Compte supprimé définitivement.', 'var(--muted2)');
...
// ── Auto-save dans le cloud après chaque modification ─────────
// On surcharge setData pour déclencher une sync cloud toutes les 30s max
let cloudSyncTimer = null;
const _origSetData = setData;
function setData(k, v) {
  _origSetData(k, v);
  // Debounce : attendre 3s d'inactivité puis sauvegarder
  clearTimeout(cloudSyncTimer);
  cloudSyncTimer = setTimeout(() => {
    if (getSbConfig()) saveToCloud();
  }, 3000);
}
...
function init() {
  ...
  document.getElementById('greetName').textContent = currentUser.charAt(0).toUpperCase() + currentUser.slice(1);
